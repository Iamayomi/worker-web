...
export function UsersTable({
  ...
  fixedAccountType?: AccountType;
  onExportReady?: (data: UsersExport) => void;
}) {
  const { user } = useAuth();
  ...
  const exportRows = useMemo(() => users.map(mapRow), [users]);
  ...
  }, [email, status, accountType]);

  const exportData = useMemo<UsersExport | null>(
    () => (onExportReady ? { rows: exportRows, fetchAll: fetchAllRows } : null),
    [onExportReady, exportRows, fetchAllRows]
  );

  useEffect(() => {
    if (exportData) onExportReady?.(exportData);
  }, [exportData, onExportReady]);
                ...
                <tr key={u.id} className="border-b border-border/10 last:border-0">
                  ...
                  <td className="px-4 py-3">
                    {u.verification_status === "verified" ? (
                      ...
                      <Badge
                        className={
                          VERIFICATION_STYLES[u.verification_status] ?? undefined
                        }
                      >
                      ...
                      <DropdownMenuContent align="end" className="w-44">
                        ...
                        {canManage && (
                          <>
                            <DropdownMenuSeparator />
                            {u.status === "suspended" ? (
                              <DropdownMenuItem onSelect={() => activateUser.mutate(u.id)}>
