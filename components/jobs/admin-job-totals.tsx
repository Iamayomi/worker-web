...
import { Briefcase, FileCheck2, UserCheck, Users } from "lucide-react";
...
const SUCCESS_STATUSES = ["offered", "accepted"];

export function AdminJobTotals() {
  const { data, isLoading } = useJobAnalytics({ days: 14 });
  if (isLoading || !data) return <StatCardsSkeleton count={4} />;

  const offersAccepted = data.applications_by_status
    .filter((s) => SUCCESS_STATUSES.includes(s.status))
