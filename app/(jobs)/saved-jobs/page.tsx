...
function SavedJobRow({ job }: { job: SavedJob }) {
 ...
 <Link
 href={`/jobs/${job.id}`}
 className="inline-flex h-10 items-center justify-center gap-1.5 border border-border px-5 text-sm font-medium transition-colors hover:border-primary hover:text-primary"
 >
...
export default function SavedJobsPage() {
 ...
 <div key={i} className="h-28 animate-pulse border border-border/15 bg-muted" />
 ))}
 </div>
 ) : jobs.length === 0 ? (
 <div className=" border border-border/15">
